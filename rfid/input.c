...
int print_event(struct input_event *event, int verbose, char *evt)
{
	char *keycode;
	...
	case EV_KEY:
		if(verbose)
			fprintf(stderr, "KEY_EV: %s %s\n", ev_type_name(EV_KEY, event->code), event->value ? "pressed" : "released");

		if (event->value != 1)
			return 0;

		if (((int)strncmp(ev_type_name(EV_KEY, event->code), enter, sizeof(enter)) == 0))
			return 1;
		
		keycode = ev_type_name(EV_KEY, event->code);
		if(verbose)
			fprintf(stderr, "keycode -> %s\nkey -> %c\n", keycode, keycode[4]);
		
		evt[strlen(evt)] = keycode[4];
		break;
	case EV_REL:
	case EV_ABS:
	case EV_MSC:
